...
def get_android_cpu_usage(pid):
    try:
        with subprocess.Popen(['adb', 'shell', 'cat /proc/stat'], stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                              text=True) as proc:
            cpu_total = sum(map(int, proc.stdout.readline().split()[1:8]))

        with subprocess.Popen(['adb', 'shell', f'cat /proc/{pid}/stat'], stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                              text=True) as proc:
...
def get_android_mem_usage(pid):
    try:
        top_output = subprocess.check_output(
            ['adb', 'shell', f'top -p {pid} -n 1 -d 0.5'],
            text=True,
...
def get_mem_from_dumpsys(pid):
    try:
        mem_output = subprocess.check_output(
            ['adb', 'shell', f'dumpsys meminfo {pid}'],
            text=True,
            stderr=subprocess.STDOUT
        )

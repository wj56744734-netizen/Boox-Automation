...
class Test_Note_Export_Time:
    ...
    def _is_overwrite_dialog_visible(self, test_note_name):
        ...
        """
        locator = f'“{test_note_name}”已存在，要替换它吗？'
        try:
            wait = WebDriverWait(self.driver, 2)
            element = wait.until(EC.visibility_of_element_located((By.XPATH, f'//*[@text="{locator}"]')))
...
#---------------- 笔记导出相关方法 -------------------
    ...
    def export_from_note_home_page(self, test_note_name):
        ...
        def export_from_notebook_home(note_name, export, export_layers=None):
            ...
            self.method.xpath_text_click(locator="分享与导出")

            # 点击导出中格式
            self.method.xpath_text_click(locator=export)
            ...
            if export != ".note文件（BOOX笔记格式）":
                ...
                # 点击导出中尺寸
                self.method.xpath_text_click(locator=export_size)

                # 点击导出中图层
                self.method.xpath_text_click(locator=export_layers)

            else:

                export_size = "无"

            self.method.xpath_text_click(locator="导出")
            ...
            if self._is_overwrite_dialog_visible(test_note_name):
                self.method.xpath_text_click(locator="确定")
            ...
            if toast:
                ...
                if result and result.get("success"):
                    log_line = result["detailed_matches"].get(
                        "com.onyx.android.note.note.action.export.ExportNoteAction", "")
        ...
        if version == "4.2":
            for export_format in new_version:
                # 长按笔记
                self.method.wait_for_press_name(by="id", locator="com.onyx:id/title", target_name=test_note_name)
    ...
    def export_from_within_note(self, test_note_name, share_and_export_guide):
        ...
        def release_share_and_export():
            ...
            self.method.xpath_text_click(locator="更多")
            time.sleep(3)
            self.method.wait_for_screen_size(x_ratio=0.17, y_ratio=0.96)
            self.method.xpath_text_click(locator="全刷")
            self.method.xpath_text_click(locator="搜索")
            self.method.xpath_text_click(locator="手势")
            self.method.xpath_text_click(locator="缩放")
            self.method.xpath_text_click(locator="页面+")
            self.method.xpath_text_click(locator="文本框")
            self.method.xpath_text_click(locator="选项设置")
            self.method.xpath_text_click(locator="分享与导出")

        self.method.xpath_text_click(locator=test_note_name)
        time.sleep(2)
        ...
        if device_region == "国内":
            ...
            try:
                ...
                if element:
                    ...
                    self.method.xpath_text_click(locator=share_and_export)
            except:
        ...
        version = self.public.get_version(short=True)
        time.sleep(3)
        self.method.wait_for_screen_size(x_ratio=0.87, y_ratio=0.03)
        ...
        self.method.wait_for_screen_size(x_ratio=0.08, y_ratio=0.04)
        self.method.by_element_click("手写笔记.退出手写笔记")

    def test_note_export_time(self, note_test_initial):

        self.method.xpath_text_click("笔记首页.进入笔记首页")
        self.method.xpath_text_click("笔记首页.无笔记状态创建按钮")
        self.method.xpath_text_click("笔记首页.从本地文件导入")
        ...
        time.sleep(3)
        self.method.xpath_text_click("笔记首页.进入笔记首页")
        self.method.xpath_text_click(locator="手写线条(1)", should_click=False)


        page_number = self.method.check_list_timeout(by="id", locator="com.onyx:id/page_info")
        for ele in page_number:
            logging.debug(f"页码元素文本：{ele.text}")

        test_note = []
        test_note_name_elements = self.method.check_list_timeout(by="id", locator="com.onyx:id/title")

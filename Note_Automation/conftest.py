...
from Note_Automation.config import driver, ensure_driver_alive
from Note_Automation.framework.health import ensure_adb_device_ready, run_adb_command_with_retry
import allure
import pytest
import time
...
def _match_deselect_pattern(nodeid, pattern):
    ...
    # 常见差异：是否包含 "Note_Automation/" 前缀
    normalized_nodeid = nodeid.removeprefix("Note_Automation/")
    normalized_pattern = pattern.removeprefix("Note_Automation/")
    if (
...
def pytest_sessionfinish(session, exitstatus):
    ...
    try:
        from Note_Automation.framework.cleanup import cleanup_artifacts
        cleanup_artifacts()

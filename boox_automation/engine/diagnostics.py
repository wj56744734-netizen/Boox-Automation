...
def expected_tag_not_matched(tag: str, found_sheets: list[str], total: int,
                              match_index: dict, module_hint: str,
    ...
    lines.append(f"  已加载有效条目: {total}")
    lines.append(f"  当前 B 列已有匹配文本({len(match_index)}条): {index_str}")
    lines.append(f"  未匹配的文本: 【{tag}】")

    # 检查是否在跳过列表中找到匹配
    if skipped:
        skipped_hits = [s for s in skipped if s.get("match") == tag]
        if skipped_hits:
            lines.append("  ────────────────────────────────────────────────")
            lines.append("  ⚠ 发现匹配但被跳过的条目:")
            for s in skipped_hits:
                lines.append(f"     第{s['row']}行 key=【{s['key']}】")
                lines.append(f"     跳过原因: {s['reason']}")
                lines.append(f"     → 修复该行数据即可解决此报错")

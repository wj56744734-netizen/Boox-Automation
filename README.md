...
编辑 `Note_Automation/excel_framework/test_cases.xlsx`，在"操作步骤"列中写：
...
详细规则 → `Note_Automation/excel_framework/使用指南.md`

### 定义页面元素

编辑 `Note_Automation/excel_framework/elements.xlsx`，告诉程序每个按钮/文字怎么找。
...
```
Note_Automation/
├── excel_framework/         # Excel 驱动测试框架（核心）
...
│
├── Note_class/              # 底层操作引擎
│   ├── Note_class.py        # 14+ 种 UI 操作（点击/输入/长按/滑动）
│   ├── element_catalog.py   # 元素中文描述
│   └── Logcat.py            # 设备日志捕获
│
├── Devices_list/            # 设备管理
│   ├── Device_basic_information.py
│   └── devices/             # 70+ 设备型号 YAML 库
│
├── framework/               # 基础设施
...
│   └── cleanup.py           # 产物清理
│
├── Test_local_notes/        # 测试套件
│   ├── test_excel_runner.py # Excel 用例 pytest 入口
│   ├── Public_method.py     # 公共业务方法
│   ├── run_test_report.py   # Allure 报告生成
│   └── Test_performance_verify/  # 性能测试
│
├── config.py                # Appium Driver 管理

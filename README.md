...
**输入变量：** 前置条件定义 `【输入用户名-testuser】`，步骤中用 `【用户名】` 引用。
...
格式：`步骤{N}：【{key}】` + 可选后缀。不含 `【】` 的行跳过，I 列为空 WARNING 但不阻塞。

| 后缀 | 模式 | 说明 |
|---|---|---|
| （无） | visible | 预期元素可见 |
| `不可见` / `不存在` | not_visible | 预期元素不可见 |
| `toast提示` | toast | 等待 Toast 弹出 |
| `toast不出现` | toast_not | 等待期内 Toast 不弹出 |

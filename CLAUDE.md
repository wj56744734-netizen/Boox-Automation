...
- **用例两阶段收集** (`test_excel_runner.py`)：全量文本解析（快速）→ 优先级 + 模块筛选 → 仅对筛选后的少量用例做元素匹配。避免对全部用例做无用匹配，显著减少收集阶段日志噪音。
- **元素表严格筛选**：`test_case_sheets` 控制加载哪些元素 sheet，仅精确匹配（不加载无关模块元素）。"通用"始终自动加载。
- **表头自动检测** (`_find_header_row()`)：通过关键字（"模块"/"匹配文本"等）自动识别表头行，兼容旧格式（注释行 + 表头行）和新格式（表头行直接在第一行）。
...
- **新增用例模块**：在飞书用例表（表 A）和元素表（表 B）中分别新建同名 sheet，同时在元素表中新建 `预期结果【模块名】` sheet。最后在 `config.yaml` 的 `excel.test_case_sheets` 列表加一行。`test_case_sheets` 同时控制用例、元素、预期结果三者的加载
...
- **元素 sheet**：Sheet 名精确匹配 `test_case_sheets` 中任一值，或精确匹配"通用"（始终加载）。非配置模块 sheet 不加载
...
**元素标识构造**：key = `模块.匹配文本`（如 模块=笔记首页，匹配文本=创建笔记按钮 → key=`笔记首页.创建笔记按钮`）。旧格式 A 列已含 '.' 时视为完整 key 不做拼接。

**元素匹配优先级**（`ElementMatcher.match()`，代码 `engine/elements.py`）：

| 优先级 | 匹配方式 | 说明 |
|---|---|---|
| 1 | `page_context.tag` 直接查找 | key 精确存在即返回 |
| 2 | `通用.tag` 回退 | 跨模块通用元素兜底 |
| 3 | 反向索引精确匹配 | B列（匹配文本）精确命中 |
| 4 | 多候选评分 | 按页面上下文评分选最优 |
| 5 | locator 纯文本精确匹配 | 非 XPath 的纯文本 locator 兜底（子串不匹配） |

**匹配失败处理**（`warn` 参数）：
- `True`（运行时）：WARNING 级别，显示在日志中
- `False`（DEBUG）：仅 DEBUG 级别，默认不显示
- `None`（静默）：不输出任何日志（用例批量收集阶段使用）

**key 末段兜底索引**：元素 B 列（匹配文本）为空时，自动取 key 末段（`.` 之后的部分）加入反向索引。如 key=`桌面进入.书库首页` → 索引"书库首页"，使 `【书库首页】` 能匹配到该元素。
...
用例 I 列格式：直接写 `【预期结果匹配文本】描述内容`，无需 `步骤N:` 前缀（H 列步骤号已自动关联）。旧格式带 `步骤N:` 前缀的仍然兼容。不含 `【】` 的行视为文档描述跳过，I 列为空不检查（兼容老用例）。

关联方式：执行时 H 列 `检查【X】` 自动去 I 列找包含 `【X】` 的行，按出现顺序一一对应。
H 列 `检查【X】` 在 I 列找不到匹配 → 报错终止该条用例，报错信息包含用例行号+标题、H 列步骤号、I 列已有的全部 `【】` 标记（方便对照修正）。
I 列的 `【X】` 在 H 列没有对应 `检查` → DEBUG 日志提示，不阻断。
...
| H 列 `检查【X】` 报错"未在用例表I列中找到匹配" | H 列【】内文字与 I 列【】不完全一致 | 比对报错中列出的 I 列已有标记，修正 H 或 I 列使其一致 |
